...
| Source systems | 13 (FDOR, City of Jacksonville, JTA, NHD, Overture, Sunbiz, DBPR) |
| Refresh | every 6 h on GitHub Actions, incremental |
| Query table | ~49 MB parquet, 103 columns |
...
flowchart LR
    ...
    llm[["Model provider<br/><i>OpenAI by default</i>"]]
...
flowchart LR
    ...
    subgraph read ["Read — on demand, no credential"]
        ...
        agent["Agent route<br/>serverless · DuckDB · 5 tools"]
...
flowchart LR
    ...
    feat[("derived.properties_features<br/>404,023 rows x 103 columns")]
...
from. `roof_age_basis` is `PERMIT`, `APPRAISER_FIELD` or `EFF_YR_BLT_PROXY`, and the UI shows which —
that is why an answer can be trusted without opening the pipeline.
...
sequenceDiagram
    ...
    alt cold instance
        ...
        Note over R,D: ~900 ms for 49 MB, then queries are local
    else warm instance
        R->>D: reuse cached instance
    end

    R->>M: question + 5 read-only tool definitions
